...
void StateHandlerCrouch(struct __CharacterState* In, int CurrentSection, int Data) // Matching - 100%
{
	struct __Event* Ptr;
	int Anim;
	struct _Instance* heldInst;
	...
	if ((!(PadData[0] & RazielCommands[6])) && (In->SectionList[CurrentSection].Process == &StateHandlerCrouch))
	{
...
void StateHandlerMove(struct __CharacterState* In, int CurrentSection, int Data) // Matching - 100%
{
	struct __Event* Ptr;
	int Anim;
	int data;
	...
	while (Ptr = PeekMessageQueue(&In->SectionList[CurrentSection].Event))
	{
		...
		case 0:
			if (CurrentSection == 0)
			{
				...
				{
					if ((Raziel.Mode == 0x2) || (Anim == 123) || (Anim == 124))
					{
						StateSwitchStateCharacterData(In, &StateHandlerIdle, SetControlInitIdleData(0, 0, 3));
					}
...
void StateHandlerDeCompression(struct __CharacterState* In, int CurrentSection, int Data) // Matching - 100%
{
	struct __Event* Ptr;
...
void StateHandlerGlide(struct __CharacterState* In, int CurrentSection, int Data) // Matching - 99.94%
{
	struct __Event* Ptr;
	int Frame;
	int Anim;
	...
	while (Ptr = PeekMessageQueue(&In->SectionList[CurrentSection].Event))
	{
		...
		case 0x4010008:
			...
			break;
		case 0x20000001:
			if (((Frame >= 13) || (Anim != 16)) && (!(PadData[0] & RazielCommands[3])))
			{
				if (CurrentSection == 0)
				{
					SetPhysics(In->CharacterInstance, -16, 0, 0, 0);
...
void StateHandlerGlyphs(struct __CharacterState* In, int CurrentSection, int Data) // Matching - 100%
{
	struct __Event* Ptr;
	int hitPosted;
	...
	while ((Ptr = PeekMessageQueue(&In->SectionList[CurrentSection].Event)) != NULL)
	{
		...
		case 0x100000:
			if (In->SectionList[CurrentSection].Data1 == 0)
			{
				if (((Raziel.Mode & 0x40000)) && (Raziel.CurrentPlane == 1))
				{
		...
		case 0x80003:
			if (CurrentSection == 0)
			{
				...
				razSetupSoundRamp(In->CharacterInstance, (struct _SoundRamp*)&Raziel.soundHandle, 19, 600, 750, 60, 120, 204800, 3500);

				Raziel.soundTimerNext = 204800;
		...
		case 0x80001:
			if (CurrentSection == 0)
			{
				...
				razSetupSoundRamp(In->CharacterInstance, (struct _SoundRamp*)&Raziel.soundHandle, 12, 0, 125, 80, 80, 315392, 3500);

				Raziel.soundTimerNext = 315392;
...
void RelocateConstrict(struct _SVector* offset) // Matching - 100%
{
	int i;
...
void TrailWaterFX(struct _Instance* instance, int Segment, int Bubbles, int Type) // Matching - 100%
{
	struct _SVector Pos;
	struct _SVector Vel;
	...
	if (instance->matrix != NULL)
	{
		if (instance->oldMatrix != NULL)
		{
			for (i = 0; i < 2; i++)
			{
				Pos.x = (short)(instance->oldMatrix[Segment].t[0] + (((instance->matrix[Segment].t[0] - instance->oldMatrix[Segment].t[0]) * i) / 2));
				Pos.y = (short)(instance->oldMatrix[Segment].t[1] + (((instance->matrix[Segment].t[1] - instance->oldMatrix[Segment].t[1]) * i) / 2));
				Pos.z = (short)(instance->oldMatrix[Segment].t[2] + (((instance->matrix[Segment].t[2] - instance->oldMatrix[Segment].t[2]) * i) / 2));

				for (j = 0; j < Bubbles; j++)
				{
					Vel.x = (short)((Pos.x - instance->oldMatrix[Segment].t[0]) + (rand() % 20)) - 10;
					Vel.y = (short)((Pos.y - instance->oldMatrix[Segment].t[1]) + (rand() % 20)) - 10;

					if (Type == 0)
					{
						Vel.z = (short)(Pos.z - instance->oldMatrix[Segment].t[2]) + (rand() % 10);
