...
void FX_DrawAllGeneralEffects(MATRIX* wcTransform, struct _VertexPool* vertexPool, struct _PrimPool* primPool, unsigned long** ot) // Matching - 100%
{
	struct _Instance* instance;
	struct _FXGeneralEffect* currentEffect;
	struct _FXGlowEffect* currentGlow;

	for (currentEffect = FX_GeneralEffectTracker; currentEffect != NULL; currentEffect = (struct _FXGeneralEffect*)currentEffect->next)
	{
		...
		if ((instance == NULL) || (!(instance->flags & 0x800)) && (!(instance->flags2 & 0x4000000)))
		{
			...
			{
				currentGlow = (struct _FXGlowEffect*)currentEffect;

				if (currentEffect->lifeTime > 0)
				{
					currentEffect->lifeTime -= (short)gameTrackerX.lastLoopTime;
			...
			{
				PIPE3D_HalvePlaneGetRingPoints(instance, wcTransform, vertexPool, primPool, ot, (struct _FXHalvePlane*)currentEffect);
			}
			else if (currentEffect->effectType == 132)
			{
				FX_DrawBlastring(wcTransform, (struct _FXBlastringEffect*)currentEffect);
			}
			else if (currentEffect->effectType == 133)
			{
				FX_LightHouse(wcTransform, ot, currentEffect->instance, ((struct _FXLightBeam*)currentEffect)->startSeg, ((struct _FXLightBeam*)currentEffect)->endSeg, 32, ((struct _FXLightBeam*)currentEffect)->color);
			}
			else if (currentEffect->effectType == 134)
			{
				FX_DrawFField(wcTransform, (struct _FXForceFieldEffect*)currentEffect);
			}
			else if (currentEffect->effectType == 135)
			{
				FX_DrawLightning((struct _FXLightning*)currentEffect, wcTransform, ot);
			}
			else if (currentEffect->effectType == 136)
			{
				FX_DrawFlash((struct _FXFlash*)currentEffect);
			}
		}
	}
}
...
struct _FXBlastringEffect* FX_DoBlastRing(struct _Instance* instance, struct _SVector* position, MATRIX* mat, int segment, int radius, int endRadius, int colorChangeRadius, int size1, int size2, int vel, int accl, int height1, int height2, int height3, long startColor, long endColor, int pred_offset, int lifeTime, int sortInWorld) // Matching - 100%
{
	...
	if (blast != NULL)
	{
		...
		blast->predator_offset = pred_offset;
		blast->predator_offset_orig = pred_offset;
		...
		blast->radius = radius << 12;
		blast->startRadius = radius << 12;
		blast->size1 = size1;
		blast->size2 = size2;
		blast->endRadius = endRadius << 12;
		blast->colorchange_radius = colorChangeRadius << 12;
		blast->vel = vel;
		blast->accl = accl;
		blast->height1 = height1;
		blast->height2 = height2;
		blast->height3 = height3;
		blast->color = startColor;
		blast->startColor = startColor;
